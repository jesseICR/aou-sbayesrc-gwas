#!/bin/bash
# run_ea_gwas.sh - Build and optionally run the AoU educational-attainment GWAS.
...
usage() {
    cat <<'EOF'
Usage: bash run_ea_gwas.sh [OPTIONS]

Builds AoU educational-attainment GWAS inputs and, unless --setup-only is
specified, runs the continuous-trait REGENIE GWAS using Step 13 genotypes.

Options:
  --setup-only       Build regenie_input/ea_gwas only; do not submit REGENIE.
  --regenie-only     Skip setup and run REGENIE against existing EA inputs.
...
export EA_GWAS_INPUT_NAME="${EA_GWAS_INPUT_NAME:-ea_gwas}"
export EA_GWAS_OUTPUT_NAME="${EA_GWAS_OUTPUT_NAME:-ea_gwas}"
export DX_EA_REGENIE_INPUT_DIR="${DX_REGENIE_INPUT_DIR}/${EA_GWAS_INPUT_NAME}"
export DX_EA_REGENIE_INPUT_URI="${DX_REGENIE_INPUT_URI}/${EA_GWAS_INPUT_NAME}"
...
export EA_QUESTION_CONCEPT_ID="${EA_QUESTION_CONCEPT_ID:-1585940}"
export EA_N_PCS="${EA_N_PCS:-10}"
...
export REGENIE_PHENO_COL="${REGENIE_PHENO_COL:-ea_years}"
if [[ -z "${REGENIE_COVAR_COLS:-}" ]]; then
    REGENIE_COVAR_COLS="yob_c,sex_c,yob_c_sex_c_inter"
    for i in $(seq 1 "${EA_N_PCS}"); do
...
mkdir -p "${SCRIPT_DIR}/logs" "${LOCAL_REGENIE_DIR}" "${DX_REGENIE_INPUT_DIR}" \
    "${DX_EA_REGENIE_INPUT_DIR}" "${DX_REGENIE_OUTPUT_DIR}" "${DX_LOGS_DIR}"

LOG_FILE="${SCRIPT_DIR}/logs/run_ea_gwas_$(date +%Y%m%d_%H%M%S).log"
exec > >(tee -a "${LOG_FILE}") 2>&1

echo "============================================"
echo "AoU EA GWAS — $(date)"
echo "============================================"
echo "  WORKSPACE_BUCKET_URI = ${WORKSPACE_BUCKET_URI}"
echo "  WORKSPACE_CDR        = ${WORKSPACE_CDR}"
echo "  EA input             = ${EA_GWAS_INPUT_NAME}"
echo "  EA output            = ${EA_GWAS_OUTPUT_NAME}"
echo "  EA question concept  = ${EA_QUESTION_CONCEPT_ID}"
...
echo "  Min survey age       = ${GWAS_MIN_AGE_AT_SURVEY}"
...
if [[ "${RUN_SETUP}" -eq 1 ]]; then
    echo "=== Setup EA GWAS inputs ==="
    bash "${SCRIPT_DIR}/setup_ea_gwas.sh"
else
    echo "=== Setup EA GWAS inputs ==="
...
if [[ "${RUN_REGENIE}" -eq 1 ]]; then
    echo ""
    echo "=== Run EA GWAS with REGENIE ==="
    bash "${SCRIPT_DIR}/run_continuous_regenie_gwas.sh" \
        "${EA_GWAS_INPUT_NAME}" "${EA_GWAS_OUTPUT_NAME}"
else
    echo ""
    echo "=== Run EA GWAS with REGENIE ==="
