#!/bin/bash
# setup_ea_gwas.sh - Build AoU educational-attainment GWAS inputs.
...
: "${DX_EA_REGENIE_INPUT_DIR:?DX_EA_REGENIE_INPUT_DIR not set}"
: "${DX_EA_REGENIE_INPUT_URI:?DX_EA_REGENIE_INPUT_URI not set}"
...
EA_QUESTION_CONCEPT_ID="${EA_QUESTION_CONCEPT_ID:-1585940}"
EA_N_PCS="${EA_N_PCS:-10}"
...
local_scrap="${LOCAL_REGENIE_DIR}/ea_gwas_scrap"
local_out="${local_scrap}/outputs"
mkdir -p "${DX_EA_REGENIE_INPUT_DIR}/scrap" "${LOCAL_REGENIE_DIR}" "${local_scrap}"
...
choose_bq_tmp_dataset() {
    ...
    echo "  Set SBAYESRC_BQ_TMP_DATASET or EA_BQ_TMP_DATASET to an existing writable dataset." >&2
    return 1
}
...
desired_params="${LOCAL_REGENIE_DIR}/ea_gwas.desired_params.tsv"
{
    ...
    printf 'fam_size\t%s\n' "$(stat -c%s "${fam}")"
    printf 'setup_ea_gwas_py_sha256\t%s\n' "$(sha256sum "${SCRIPT_DIR}/setup_ea_gwas.py" | awk '{print $1}')"
} > "${desired_params}"

params="${DX_EA_REGENIE_INPUT_DIR}/ea_gwas.params.tsv"
summary="${DX_EA_REGENIE_INPUT_DIR}/ea_gwas.summary.tsv"
phen="${DX_EA_REGENIE_INPUT_DIR}/phen.txt"
base_covar="${DX_EA_REGENIE_INPUT_DIR}/base_covar.txt"
covar="${DX_EA_REGENIE_INPUT_DIR}/covar.txt"
training_iids="${DX_EA_REGENIE_INPUT_DIR}/training_iids.txt"
answer_counts="${DX_EA_REGENIE_INPUT_DIR}/ea_answer_counts.tsv"
log_file="${DX_EA_REGENIE_INPUT_DIR}/ea_gwas_log.txt"
...
if [[ -s "${params}" && -s "${summary}" && -s "${phen}" && -s "${base_covar}" &&
    ...
    if diff -q "${desired_params}" "${params}" >/dev/null 2>&1; then
        ...
        if [[ -n "${expected}" && "${phen_rows}" -eq "${expected}" &&
            ...
            echo "  EA GWAS inputs already exist (${expected} samples) — skipping"
            exit 0
        fi
    fi
    echo "  EA GWAS inputs exist but params/counts do not match — rebuilding"
fi

ea_query="${local_scrap}/ea_query.csv"
query_sql="${local_scrap}/ea_query.sql"
query_log="${local_scrap}/ea_query.log"
tmp_dataset="$(choose_bq_tmp_dataset "${EA_BQ_TMP_DATASET:-${SBAYESRC_BQ_TMP_DATASET:-}}")"
tmp_result_table="ea_gwas_result_$(date +%Y%m%d_%H%M%S)_$$"
tmp_result_ref="${GOOGLE_PROJECT}:${tmp_dataset}.${tmp_result_table}"
ea_query_gs="${DX_EA_REGENIE_INPUT_URI}/scrap/ea_query.csv"
...
WITH codeable AS (
  SELECT
    ...
    EXTRACT(YEAR FROM DATE(p.birth_datetime)) +
      SAFE_DIVIDE(
        DATE_DIFF(
          ...
          DATE(EXTRACT(YEAR FROM DATE(p.birth_datetime)), 1, 1),
          DAY
        ),
        DATE_DIFF(
          DATE(EXTRACT(YEAR FROM DATE(p.birth_datetime)) + 1, 1, 1),
          DATE(EXTRACT(YEAR FROM DATE(p.birth_datetime)), 1, 1),
          DAY
        )
      ) AS yob,
    ...
    s.answer AS answer,
    s.survey_datetime,
    COUNT(*) OVER (PARTITION BY s.person_id) AS n_ea_records,
    ROW_NUMBER() OVER (
  ...
  WHERE s.question_concept_id = ${EA_QUESTION_CONCEPT_ID}
...
echo "  Querying AoU CDR for EA phenotype into temporary BigQuery table ${tmp_result_ref} ..."
...
echo "  Exporting EA query result to workspace bucket ..."
gcloud storage rm "${ea_query_gs}" \
...
bq --project_id="${GOOGLE_PROJECT}" extract \
    ...
    "${ea_query_gs}" \
    >> "${query_log}" 2>&1
gcloud storage cp "${ea_query_gs}" "${ea_query}" \
    --billing-project="${GOOGLE_PROJECT}" >> "${query_log}" 2>&1

ea_query_rows=$(( $(wc -l < "${ea_query}") - 1 ))
echo "  BigQuery EA rows after codeable-answer/birth-date filters: ${ea_query_rows}"
if [[ "${ea_query_rows}" -le 0 ]]; then
    echo "ERROR: EA query returned no rows" >&2
    exit 1
fi
...
python3 "${SCRIPT_DIR}/setup_ea_gwas.py" \
    --ea-query "${ea_query}" \
    --europeans "${europeans}" \
...
cp "${desired_params}" "${local_out}/ea_gwas.params.tsv"

echo "  Copying EA GWAS inputs to workspace bucket ..."
gcloud storage cp \
    "${local_out}/training_iids.txt" \
    "${local_out}/phen.txt" \
    "${local_out}/base_covar.txt" \
    "${local_out}/covar.txt" \
    "${local_out}/ea_answer_counts.tsv" \
    "${local_out}/ea_gwas.summary.tsv" \
    "${local_out}/ea_gwas_log.txt" \
    "${local_out}/ea_gwas.params.tsv" \
    "${DX_EA_REGENIE_INPUT_URI}/" \
    --billing-project="${GOOGLE_PROJECT}" >/dev/null

echo "  EA GWAS setup summary:"
